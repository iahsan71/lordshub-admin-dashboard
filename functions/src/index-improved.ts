/**
 * IMPROVED VERSION with Firestore-based thread mapping
 * This version persists thread mappings in Firestore instead of memory
 * Use this for production to handle function cold starts
 */
...
/**
 * Find session ID from message text
 */
  ...
  try {
    ...
    if (message.reply_to_message) {
      ...
      await bot.sendMessage(
        ADMIN_TELEGRAM_ID,
        "ℹ️ To reply to a customer, please reply to their message.\n\n" +
        "Don't send a new message - use the Reply feature in Telegram."
      );
    }
...
/**
 * Cleanup old thread mappings (run monthly)
 */
export const cleanupOldThreads = functions.pubsub
  ...
  .onRun(async () => {
    try {
      ...
      console.log(`Cleaned up ${snapshot.size} old thread mappings`);
    } catch (error) {
